...
> **Current roadmap (2026-09-06).** Phases 16–19 are complete; Phase 20 below is next.
...
> ([ADR-0046](decisions/0046-splitwise-audit-findings-and-external-read-completeness.md)).
> Phase 20's derived proof packs are next.
...
**Phase 20: Derived Proof Packs.** Read
[ADR-0046](decisions/0046-splitwise-audit-findings-and-external-read-completeness.md),
[ADR-0018 (item refunds)](decisions/0018-item-level-refund-attribution.md) and `CLAUDE.md`'s
sixth pillar before implementation. Every number a pack quotes already exists and is already
derived once — `domain.netAmount`, the current refund-aware allocation, `computeNetBalance`,
`obligationEvidenceStatus` — so the phase is a _rendering and redaction_ problem, not a
second financial engine, and a pack that recomputes a share is a bug. Two things phase 19 makes
newly relevant: a pair with an open audit finding has facts that are not settled, and a pack
must show that rather than quote a confident balance over it; and the same "an incomplete check
is not agreement" rule applies to a pack's own as-of timestamp. Generating a pack neither sends
it nor records a settlement, and the recipient preview/redaction step is separate from the
local evidence boundary that let the pack be built at all.
