...
> **Current roadmap (2026-09-06).** Phases 16, 17 and 18 are complete; Phase 19 below is next.
...
> **shipped**. Phase 19's Splitwise drift and ghost-debt auditing is next.
...
**Phase 19: Splitwise Drift & Ghost-Debt Auditing Engine.** Read
[ADR-0041](decisions/0041-reconciliation-drift-detection-scope.md),
[ADR-0040](decisions/0040-phase-14-scope-is-first-sync-not-full-lifecycle.md) and `CLAUDE.md`
before implementation. Phase 15 already ships pair-level comparison against Splitwise's
balances, and Phase 18 has just made unreflected item refunds a real source of drift — a
`SplitwiseExpense` moved to `stale` by an item-attributed distribution is exactly the
"apparent debt with no support in the current local ledger" Phase 19 has to surface. Inspect
`services.runReconciliation`, `domain.splitwise-drift` and `markSplitwiseExpenseStale`/
`markSplitwiseExpenseDrifted` before adding anything: the aggregate comparison exists and must
be extended rather than rebuilt, and `stale` (our side changed) must stay distinct from
`drifted` (theirs did).
