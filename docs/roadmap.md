...
**Phase 8, Transaction classification.** Phase 7 is done, so payments now sit at `normalized`
with a refined `channel` and, where the catalog knew them, a resolved merchant. The next slice is
the one that decides what a payment _is_ — `ai.classifyTransaction` proposing
`proposedKind: expense | settlement` (ADR-0007), `Expense`/`Settlement` creation, and
confidence-based routing into `REVIEW_REQUIRED`.

Phase 8 is where the **AI service boundary itself gets built**, not merely used: the typed
interface, the structured-proposal contract, `confidence` handling, the validation gate, and
`decideInference()` as the sole path by which an `AIInference` leaves `pending`
(`ai-boundary.md`). `src/ai/` currently holds only a `README.md` and `ai_inferences` has no
writer anywhere, so this is first construction rather than extension. ADR-0022 records why that
work was kept out of phase 7 instead of being started twice.

Four things worth carrying into it:

- **The `ai.normalizeMerchant()` leg of `data-flow.md` step 2 is still unbuilt** (ADR-0022). It
  belongs on the miss path — where `findMerchantByAliasKey` returns `null` — and nothing in phase
  7 needs revisiting when it lands. Whether it ships with phase 8 or as its own slice is open;
  what is settled is that it waits for the boundary, not the reverse.
- **The merchant catalog has no production seeding path.** `fixtures/merchants.json` is seeded
  only by the test harness (`tests/support/ledger.ts`'s `seedMerchants`), exactly as the
  synthetic cast is. There is deliberately no `insertMerchant`/`insertMerchantAlias` repository
  function, because phase 7 had no caller for one. Any real use of merchant resolution needs that
  path built first.
- **Two of the fixture's unresolved rows are classification questions, not merchant questions.**
  The two `NEFT/N072026001` legs are a self-transfer and `UPI-FRIENDA-TRANSFER` is a person;
  normalization deliberately leaves both `unknown`. They are the natural first tests for
  `counterparty_type = internal_account` and `person`, neither of which phase 7 ever writes.
- **Possible-duplicate surfacing is still phase 9's.** A row that merely _resembles_ another
  (same amount and date, no matching reference) is left alone by the importer; surfacing those
  for human confirmation is review-queue work.
