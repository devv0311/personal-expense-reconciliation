...
## After phase 22: the unnumbered capabilities

Phase 22 ended the numbered sequence and left a short list of things that were genuinely
unbuilt rather than merely unreachable ([ADR-0050](decisions/0050-closing-the-audit-gaps-a-workflow-is-not-shipped-until-it-is-reachable.md)).
Each needed its own ADR before it started, and each has one. They are not phases: nothing below
...
- **Rules/learning beyond what phase 22 shipped.** Manual rules exist and propose by default; no
  auto-approval may be inferred from confidence.
