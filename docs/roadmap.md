...
> numbering and order are unchanged; phase *descriptions* for 12, 14, and 15 below are updated to
...
- **Inflow-side reconciliation is an explicit V1 scope boundary, not an unmodeled gap**
  ...
  received-settlement credits are already fully modeled — only *plain, otherwise-unclassified*
...
- **A settlement between two people, neither of whom is the user**, can never be backed by a
  ...
  represents the obligation correctly; only its *discharge* is unobservable without Splitwise or
