...
### Phase 21 — Modern UI/UX overhaul in `web/` (delivered)

Read [ADR-0048](decisions/0048-phase-21-ui-reads-the-ledger-and-never-recomputes-it.md),
[ADR-0049](decisions/0049-keyboard-first-navigation-never-completes-a-decision.md),
...
**The whole phase turns on one rule: `web/` renders figures, it never derives them.** Where a
number the UI needed did not exist over HTTP, the _read_ was added to the API rather than the
arithmetic to the browser — so the last numbered phase's entire backend footprint is four reads
and one request field, and a screen cannot drift from the ledger because it has nothing of its
own to drift with. That is checkable and was checked: a driven browser compared 42 rendered
figures against live API responses, and the frontend suite asserts the same agreement against
fixtures.

What shipped:

- **Six sections, one product.** Review (the triage inbox), Reconciliation, Expenses, Balances,
  Splitwise and Proof packs, with evidence, payment-context and audit-finding detail screens
  reached from them rather than given tabs of their own — a document is always about a payment
  or an expense, and a list of loose documents is not a workflow.
- **The account cash waterfall** (`GET /api/reconciliation/runs/:id/account-snapshots`), showing
  ADR-0017's second identity term by term, with the internal-transfer and explained/unexplained
  figures labelled as the subsets they are (17.3). Statement boundaries are collected on the run
  form (`accountBoundaries`, signed minor units, each balance refused without its statement
  evidence per 17.5). **A verified zero is never shown over incomplete evidence**: a missing
  balance reads "not evidenced", `expectedEndingBalance` reads "cannot be computed",
  `verificationStatus` comes from the database `CHECK` rather than the screen, and a zero delta
  over unexplained debits renders in `debit` under the word "Unreconciled".
- **Context re-attachment as a workflow**: the evidence inspector shows the immutable source
  verbatim beside its structured reading, and each candidate with all six signals' verdicts and
  both compared values. Nothing auto-links; accepting is a dialog that says the link is
  permanent and write-once before it makes one.
- **The item-refund splitter**: per-item paid/refunded/net from `getRefundAllocationState`, an
  entry check over the amounts you type (labelled as one), and then the server's own
  `projectedLines` offered for approval — including the zero-amount line a fully refunded share
  keeps. A pending distribution says the obligations below it are not current or verified; a
  refund the ledger cannot allocate shows `REFUND_ITEM_OWNERSHIP_REQUIRED` instead of a button.
- **The Splitwise audit surface**: an external-read banner that calls a failed, partial or
  unsupported read an incomplete check rather than agreement, findings separated into
  disagreement / observability limit / incomplete check, an unattributed mismatch that says
  nothing explains it, and a review flow that records a conclusion and states — in the dialog —
  that it authorizes no write to Splitwise.
- **The proof-pack preview**, with every warning above the message rather than below it, the
  text shown verbatim, and copying gated behind an explicit recipient/content/evidence review.
  Copying still sends nothing and records nothing.
- **Keyboard-first navigation** (ADR-0049): `Cmd+K`/`Ctrl+K` opens a `combobox`/`listbox`
  palette, `?` lists every shortcut including the open screen's own, `g`-prefixed pairs
  navigate, `j`/`k`/`Enter` triage the queue. All of it is navigation — **no shortcut completes
  a decision**, and every consequential act is a button behind a dialog that states its
  consequence and requires a reason where the service does.
- **One new primitive and no new dependency**: a hand-built `Dialog` (focus containment,
  restoration, Escape). Two accessibility defects were fixed at the token and primitive level
  rather than per screen — `--ink-faint` darkened to meet AA on both surfaces in both themes,
  and `Table` making an overflowing scroll container keyboard-reachable, named by its caption.

Verification: 122 new frontend tests (159 total, from 37) and 14 new backend tests (1,762, from
1,748); the full gate green in both packages (`typecheck`/`lint`/`format:check`/`db:check`/
`build`/`test`); axe reporting **0 violations** across all eleven screens in desktop light,
desktop dark and mobile; no horizontal overflow and no console errors at 360px or 1280px; every
transition collapsing to ~0 under `prefers-reduced-motion`. CI gained a `web` job, closing the
frontend gap Phase 15 left open deliberately.

Deliberately **not** done: WhatsApp or any message transport, Splitwise re-sync/repair, rule
learning or auto-approval, an editor for a model's stored proposal (`modify` exists on the API
and is not reachable from the UI — a half-built editor for it would be a way to approve
something nobody read), and any new financial engine.

### Later work without assigned phase numbers

With phases 1–21 complete, everything below is what remains — and none of it is numbered,
because none of it is a prerequisite for anything already shipped.
...
pack does not implicitly authorize outbound writes — and Phase 21 did not change that: its
review flows record conclusions and its proof-pack preview copies text, neither of which
reaches an external system.

Two more, made visible by Phase 21 rather than created by it:
...
- **An editor for a model's stored proposal.** `POST /api/review/inferences/:id/decision`
  ...
  of design work.
- **A recipient-facing send step.** A proof pack can be previewed, reviewed and copied. Sending
  it — through WhatsApp or anything else — is a transport this system still does not have, and
  ADR-0047's "generating a pack neither sends it nor records a settlement" holds unchanged.
