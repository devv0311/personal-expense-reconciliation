...
| 8   | Friend pays restaurant, user owes share               | §27         | Friend A                      | Dev owes Friend A                                 | Same as above                                                                              | Explained expense, not explained _outflow_ (ADR-0016), evidence-only                                                               | Syncs                                   |
...
| 10  | Flatmate sends settlement to user                     | §29         | n/a                           | Discharges the reverse obligation                 | `Settlement`, `payment.direction = credit`                                                 | Not spend; a credit, so outside `ledger_settlements_total`'s outflow scope (ADR-0016)                                              | `SplitwiseSettlement`                   |
...
| 19  | Expense with no bank payment, someone else paid       | §26, §27    | Flatmate/Friend               | Owed to that payer                                | n/a until settled                                                                          | Explained expense, but **not** explained _outflow_ — contributes 0 to `ledger_explained_total`, no `PaymentExpenseLink` (ADR-0016) | Syncs                                   |
