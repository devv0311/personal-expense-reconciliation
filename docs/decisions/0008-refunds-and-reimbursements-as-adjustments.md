...
The original refund design created a *second* `Expense` for every refund
...
exactly"). The reference fixture for a partial refund showed a new Allocation on the *original*
...
- `kind`: `merchant_refund` (money back from the *same* counterparty as the original expense) or
...
**Invariant #11, corrected:** the sum of the expense's *current* `Allocation`'s
...
It was never well-defined as a way of classifying a *new* expense, because it isn't one — like a
...
adjustment moves to a new `stale` sync status (distinct from `drifted`, which means *Splitwise's*
side changed) — see `lifecycle.md` and `scenario-analysis.md` §30. The shape of the *fully*-
...
- **Keep refund and reimbursement as separate tables.** Considered, since the user asked for a
  ...
  exclusion) is identical for both; only the *source* of the money differs, which a `kind` column
