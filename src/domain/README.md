...
| `proof-pack.ts`              | Recipient-specific proof pack: arranges already-derived figures into prose, isolates the recipient, carries uncertainty (ADR-0047) |
| `splitwise-remote-change.ts` | What somebody changed _in_ Splitwise, as a proposal with a declared effect; absence only counts under a complete read (ADR-0056)   |
| `ledger-query.ts`            | The closed set a question may become, and the shape of an answer built from reads that already exist (ADR-0057)                    |
